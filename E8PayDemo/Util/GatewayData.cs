...
namespace E8PayDemo.Util
{
    ...
    public class GatewayData
    {
        ...
        public SortedDictionary<string, object> Pairs => _values;
        ...
        public bool Add(string key, object value)
        {
            ...
            if (value is null || string.IsNullOrEmpty(value.ToString()) || value == DBNull.Value)
            {
                return false;
            }
        ...
        /// <returns></returns>
        public string ToUrl(bool isUrlEncode = false)
        {
            return string.Join("&",
                _values.Select(a => $"{a.Key}={(isUrlEncode ? WebUtility.UrlEncode(a.Value.ToString()) : a.Value.ToString())}"));
        }
        ...
        public void FromIDictonary(IDictionary<string, string> pairs)
        {
            Clear();
            pairs.ForEach(f => _values.Add(f.Key, f.Value));
        }
        ...
        /// <returns></returns>
        public void FromNameValueCollection(NameValueCollection nvc, bool isUrlDecode = false)
        {
            foreach (var item in nvc.AllKeys)
            {
                Add(item, isUrlDecode ? WebUtility.UrlDecode(nvc[item]) : nvc[item]);
            }
        }
